...
use tickerforge::{
    load_spec, parse_ticker, parse_ticker_date, parse_ticker_date_spec, parse_ticker_spec,
...
fn generate_and_parse_round_trip() {
    ...
    let parsed = parser.parse_date(&generated, "2026-06-01").expect("parse");
...
fn parse_ticker_full_dol() {
    let parsed = parse_ticker("DOLK26").expect("parse");
...
fn parse_ticker_full_win() {
    let parsed = parse_ticker("WINM26").expect("parse");
...
fn parse_ticker_root_symbol_uses_today() {
    let parsed = parse_ticker("IND").expect("parse");
    assert_eq!(parsed.symbol, "IND");
...
fn parse_ticker_unknown_errors() {
    let result = parse_ticker("ZZZZ");
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Unable to parse ticker"));
}

// ===========================================================================
// Free functions: parse_ticker_date
// ===========================================================================

#[test]
fn parse_ticker_date_full_ignores_date() {
    let parsed = parse_ticker_date("INDM26", "1990-01-01").expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert_eq!(parsed.year, 2026);
    assert_eq!(parsed.month, 6);
}
...
fn parse_ticker_date_root_dol() {
    let parsed = parse_ticker_date("DOL", "2026-04-15").expect("parse");
    assert_eq!(parsed.symbol, "DOL");
    assert_eq!(parsed.year, 2026);
    assert!(parsed.month >= 1 && parsed.month <= 12);
}
...
fn parse_ticker_date_root_win() {
    let parsed = parse_ticker_date("WIN", "2026-04-15").expect("parse");
    assert_eq!(parsed.symbol, "WIN");
    assert_eq!(parsed.year, 2026);
    assert!(parsed.month >= 1 && parsed.month <= 12);
}
...
// ===========================================================================
// Free functions: parse_ticker_spec / parse_ticker_date_spec
// ===========================================================================

#[test]
fn parse_ticker_spec_full() {
    let spec = load_spec().expect("spec");
    let parsed = parse_ticker_spec("INDM26", &spec).expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert_eq!(parsed.year, 2026);
    assert_eq!(parsed.month, 6);
}
...
fn parse_ticker_spec_root() {
    let spec = load_spec().expect("spec");
    let parsed = parse_ticker_spec("IND", &spec).expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert!(parsed.month >= 1 && parsed.month <= 12);
}
...
fn parse_ticker_spec_unknown_errors() {
    let spec = load_spec().expect("spec");
    let result = parse_ticker_spec("ZZZZ", &spec);
    assert!(result.is_err());
}
...
fn parse_ticker_date_spec_full() {
    let spec = load_spec().expect("spec");
    let parsed = parse_ticker_date_spec("DOLK26", "2026-04-15", &spec).expect("parse");
    assert_eq!(parsed.symbol, "DOL");
    assert_eq!(parsed.year, 2026);
    assert_eq!(parsed.month, 5);
}
...
fn parse_ticker_date_spec_root() {
    let spec = load_spec().expect("spec");
    let parsed = parse_ticker_date_spec("IND", "2026-06-01", &spec).expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert_eq!(parsed.year, 2026);
    assert!(parsed.month >= 1 && parsed.month <= 12);
}
...
fn parse_ticker_date_spec_unknown_errors() {
    let spec = load_spec().expect("spec");
    let result = parse_ticker_date_spec("ZZZZ", "2026-01-01", &spec);
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Unable to parse ticker"));
}
...
fn ticker_parser_try_new() {
    ...
    let parsed = parser.parse("IND").expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert!(parsed.month >= 1 && parsed.month <= 12);
}
...
fn ticker_parser_parse_date_full_ignores_date() {
    let parser = TickerParser::new();
    let parsed = parser.parse_date("INDM26", "1990-01-01").expect("parse");
    assert_eq!(parsed.symbol, "IND");
    assert_eq!(parsed.year, 2026);
    assert_eq!(parsed.month, 6);
}
...
fn builder_build_custom_spec() {
    let parser = TickerParser::builder()
        ...
        .build()
        .expect("build");
    let parsed = parser.parse("DOLK26").expect("parse");
    assert_eq!(parsed.symbol, "DOL");
    assert_eq!(parsed.year, 2026);
    assert_eq!(parsed.month, 5);
}
...
#[test]
fn parse_full_ticker_with_date_warns_but_succeeds() {
    // The warning goes to stderr; we just verify the result is correct
    let parsed = parse_ticker_date("WINQ25", "2030-01-01").expect("parse");
    assert_eq!(parsed.symbol, "WIN");
